...
export default function WishlistPage() {
        ...
        {items.map(product => (
          <div key={product.id} className="rounded-[32px] border border-white/10 bg-slate-950/90 p-6 shadow-2xl">
            <div className="overflow-hidden rounded-3xl bg-slate-900">
              <img src={product.images[0]} alt={product.name} className="h-56 w-full object-cover transition duration-300 hover:scale-105" />
            </div>
            <div className="mt-5 space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                {product.compareAtPrice ? <p className="text-sm line-through text-slate-500">${product.compareAtPrice.toFixed(2)}</p> : null}
              </div>
  ...
  }, [user, wishlistIds.length, isLoading, items]);
